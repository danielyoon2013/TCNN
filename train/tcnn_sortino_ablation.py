...
VERSION 7 with SORTINO LOSS:
- Precomputed tensors (X_panel, Y_panel) for fast training
- 7-channel input (ret, vol_ewma, ret_norm, cs_norm_ret, cs_rank, temporal_rank_63, temporal_rank_252)
...
- SORTINO LOSS: Optimizes Sortino ratio instead of Sharpe ratio
  (penalizes downside deviation only, not upside volatility)

New V7 features (compared to V6):
- Cross-sectional normalized return (z-score across stocks each day)
- Cross-sectional percentile rank (0-1) each day
- Temporal rank 63-day (percentile rank vs past 63 days, same stock)
- Temporal rank 252-day (percentile rank vs past 252 days, same stock)

Execution assumption: observe close, execute at close auction, hold until next month-end.

Workflow:
1. Run data_download_v2.py (once)
2. Run prep_tensor_v2.py (once)
...
class TCNEncoder(nn.Module):
    ...
    7-channel input:
      ...
      - ret_norm: volatility-normalized return
      - cs_norm_ret: cross-sectional z-score of return
      - cs_rank: cross-sectional percentile rank (0-1)
      - temporal_rank_63: percentile rank vs past 63 days
      - temporal_rank_252: percentile rank vs past 252 days
    """
    def __init__(self, in_ch=7, hidden=32, d=16, k=5, dilations=(1, 2, 4, 8), dropout=0.3):
        super().__init__()
        if k % 2 == 0:
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        feats = torch.cat([last, mean, mx], dim=-1)
        h = self.proj(feats)
        return h
...
def compute_sortino_ratio(returns, annualization=252.0):
    """Compute Sortino ratio from returns tensor.

    Sortino = mean(returns) / downside_deviation * sqrt(annualization)
    downside_deviation = sqrt(mean(negative_returns^2))

    This only penalizes negative returns, not positive volatility.
    """
    mu = returns.mean()
    ...
    if len(negative_rets) > 1:
        # Standard downside deviation formula
        downside_dev = torch.sqrt((negative_rets ** 2).mean()) + 1e-6
    elif len(negative_rets) == 1:
        downside_dev = torch.abs(negative_rets[0]) + 1e-6
    else:
        # No negative returns - use small value to avoid division by zero
        # This gives a very high Sortino (good!)
        downside_dev = torch.tensor(1e-6, device=returns.device)

    sortino = mu / downside_dev * torch.sqrt(torch.tensor(annualization, device=returns.device))
...
class PrecomputedDataset:
    ...
    def __init__(self, X_panel, Y_panel, holding_days, valid_mask, rebal_dates,
        ...
        """
        Args:
            X_panel: (num_months, num_stocks, 7, lookback) - full panel
    ...
    def get_month_data(self, idx):
        ...
        Returns:
            X_t: (num_valid_stocks, 7, lookback) tensor
...
# ============================================================
# TRAINING WITH SORTINO LOSS
# ============================================================

def train_one_epoch(encoder, head, dataset, opt, batch_size, device, clip_returns=True, max_grad_norm=5.0):
    """Train for one epoch using SORTINO loss."""
    encoder.train()
    head.train()
    ...
    n_batches = (num_months + batch_size - 1) // batch_size
    ...
    for batch_idx in range(n_batches):
        ...
        batch_end = min(batch_start + batch_size, num_months)
        ...
        # SORTINO LOSS instead of Sharpe
        batch_sortino = compute_sortino_ratio(batch_rets, annualization=252.0)
        batch_sortino = torch.clamp(batch_sortino, -10.0, 10.0)
        ...
        if total_norm > 250.0:
            print(f"  Skipping batch {batch_idx}, gradient norm: {total_norm:.2f}")
            opt.zero_grad()
            continue
        ...
        del batch_returns, batch_rets, loss, batch_sortino
        if device == 'cuda':
...
def train_model_for_year(
    ...
    """Train model for a specific year using precomputed tensors and SORTINO loss."""
    print(f"\n{'='*80}")
    print(f"TRAINING MODEL FOR YEAR {year_label} (SORTINO LOSS)")
    print(f"{'='*80}")
    ...
    print("\nInitializing model...")
    encoder = TCNEncoder(
        in_ch=model_config.get('in_ch', 7),
    ...
    print(f"\nTraining with SORTINO loss...")
    print(f"{'Epoch':<8} {'Train Sort':<12} {'Val Sort':<12} {'Val MA':<12} {'Grad Norm':<12} {'LR':<12} {'Status':<20}")
    print("=" * 100)
    ...
    for epoch in range(max_epochs):
        ...
        if len(epoch_returns) > 0:
            epoch_returns_np = np.array(epoch_returns)
            negative_rets = epoch_returns_np[epoch_returns_np < 0]
            if len(negative_rets) > 0:
                downside_dev = np.sqrt((negative_rets ** 2).mean()) + 1e-8
            else:
                downside_dev = 1e-8
            epoch_sortino = epoch_returns_np.mean() / downside_dev * np.sqrt(252)
            train_sortinos.append(epoch_sortino)
        else:
            train_sortinos.append(0.0)

        val_results = evaluate_model(encoder, head, val_ds, device)
        if len(val_results) > 0:
            val_returns = np.array([r['return'] for r in val_results])
            negative_val = val_returns[val_returns < 0]
            if len(negative_val) > 0:
                val_downside = np.sqrt((negative_val ** 2).mean()) + 1e-8
            else:
                val_downside = 1e-8
            val_sortino = val_returns.mean() / val_downside * np.sqrt(252)
            val_sortinos.append(val_sortino)
        else:
            val_sortinos.append(-999)

        if len(val_sortinos) >= ma_window:
            val_sortino_ma = np.mean(val_sortinos[-ma_window:])
        else:
            val_sortino_ma = val_sortinos[-1]
        val_sortinos_ma.append(val_sortino_ma)

        scheduler.step(val_sortino_ma)
        current_lr = opt.param_groups[0]['lr']

        status = ""
        if epoch >= warmup_epochs:
            if val_sortino_ma > best_val_sortino_ma:
                best_val_sortino_ma = val_sortino_ma
                best_epoch = epoch
                epochs_without_improvement = 0
                best_encoder_state = {k: v.cpu().clone() for k, v in encoder.state_dict().items()}
                best_head_state = {k: v.cpu().clone() for k, v in head.state_dict().items()}
        ...
        print(f"{epoch+1:<8} {train_sortinos[-1]:<12.3f} {val_sortinos[-1]:<12.3f} {val_sortino_ma:<12.3f} {avg_grad_norm:<12.3f} {current_lr:<12.2e} {status:<20}")
    ...
    if best_epoch >= warmup_epochs:
        ...
        best_epoch = len(train_sortinos) - 1
        best_val_sortino_ma = val_sortinos_ma[-1] if val_sortinos_ma else -999

    print(f"Training complete for year {year_label}")
    print(f"  Best epoch: {best_epoch + 1}")
    print(f"  Best val Sortino (MA): {best_val_sortino_ma:.3f}")

    training_history = {
        'train_sortinos': train_sortinos,
        'val_sortinos': val_sortinos,
        'val_sortinos_ma': val_sortinos_ma,
        'grad_norms': avg_grad_norms,
        'best_epoch': best_epoch,
        'best_val_sortino_ma': best_val_sortino_ma,
    }
...
def test_model_on_year(
    ...
    if len(year_results) > 0:
        ...
        # Also compute Sortino for reporting
        negative_rets = year_returns[year_returns < 0]
        if len(negative_rets) > 0:
            sortino = year_returns.mean() / (np.sqrt((negative_rets ** 2).mean()) + 1e-8) * np.sqrt(252)
        else:
            sortino = float('inf')
...
def run_rolling_yearly_experiment(
    ...
    cache_dir: str = "./rolling_yearly_cache_v7_sortino",
    device: str = "cuda",
    batch_size: int = 60,
    max_epochs: int = 60,
    patience: int = 15,
    ...
    """Rolling yearly retraining with precomputed tensors and SORTINO loss."""
    print(f"\n{'='*80}")
    print(f"ROLLING YEARLY RETRAINING EXPERIMENT V7 (SORTINO LOSS)")
    print(f"{'='*80}")
    ...
    print(f"Loss function: SORTINO RATIO (penalizes downside only)")
    ...
    if len(all_results_df) > 0:
        ...
        # Overall Sortino
        negative_all = all_returns[all_returns < 0]
        if len(negative_all) > 0:
            overall_sortino = all_returns.mean() / (np.sqrt((negative_all ** 2).mean()) + 1e-8) * np.sqrt(252)
        else:
            overall_sortino = float('inf')
        ...
        for year in range(start_year, end_year + 1):
            ...
            if len(year_data) > 0:
                ...
                negative_year = year_rets[year_rets < 0]
                if len(negative_year) > 0:
                    year_sortino = year_rets.mean() / (np.sqrt((negative_year ** 2).mean()) + 1e-8) * np.sqrt(252)
                else:
                    year_sortino = float('inf')
    ...
    with open(f"{cache_dir}/training_histories.json", 'w') as f:
        ...
        for year, hist in all_training_histories.items():
            histories_serializable[year] = {
                'train_sortinos': [float(x) for x in hist['train_sortinos']],
                'val_sortinos': [float(x) for x in hist['val_sortinos']],
                'val_sortinos_ma': [float(x) for x in hist['val_sortinos_ma']],
                'grad_norms': [float(x) for x in hist['grad_norms']],
                'best_epoch': int(hist['best_epoch']),
                'best_val_sortino_ma': float(hist['best_val_sortino_ma']),
            }
...
if __name__ == "__main__":
    ...
    # V7: Use v2 precomputed tensors with 7 channels
    tensor_dir = os.path.join(script_dir, "precomputed_tensors_v2")
    parquet_path = os.path.join(script_dir, "daily_data_sf_v2.parquet")
    cache_dir = os.path.join(script_dir, "rolling_yearly_cache_v7_sortino")
    device = "cuda" if torch.cuda.is_available() else "cpu"

    print(f"Device: {device}")

    model_config = {
        'in_ch': 7,  # 7 channels: ret, vol_ewma, ret_norm, cs_norm_ret, cs_rank, temporal_rank_63, temporal_rank_252
        'hidden': 48,
        'd': 96,
        'kernel_size': 5,
    ...
    print("\nExperiment Configuration V7 (SORTINO LOSS):")
    ...
    print("  - 7-channel input:")
    print("    1. ret (raw return)")
    print("    2. vol_ewma (EWMA volatility)")
    print("    3. ret_norm (volatility-normalized return)")
    print("    4. cs_norm_ret (cross-sectional z-score)")
    print("    5. cs_rank (cross-sectional percentile 0-1)")
    print("    6. temporal_rank_63 (percentile rank vs past 63 days)")
    print("    7. temporal_rank_252 (percentile rank vs past 252 days)")
    ...
    print("  - SORTINO LOSS: Optimizes for Sortino ratio (downside risk only)")
