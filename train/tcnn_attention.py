"""
TCNN portfolio paper — single-head additive attention pooling variant.

Trained end-to-end on a Sharpe objective for cross-sectional dollar-neutral L/S
portfolios (top-2000 US equities, monthly rebal). Reproduces paper Table 1
"TCNN (Attention Pooling)" — Sharpe ≈ 0.61 OOS 2010-2023.

PATCHES applied vs. authoring repo:
    - argparse for all paths and key hyperparameters (no hardcoded Windows paths)
    - cudnn.deterministic=True; benchmark=False; deterministic algos on
    - paper-faithful defaults: in_ch=3, hidden=48, d=96, dilations (1,2,4,8,16),
      kernel=5, dropout=0.15, weight_decay=0.15, lr=8e-4
    - rolling 8yr-train + 2yr-val, max_epochs=35, patience=10, warmup=7
    - --smoke flag for fast CPU sanity test (1 year, 2 epochs)

Usage:
    # Paper-config rerun on cloud A100:
    python tcnn_attention.py \\
        --tensor-dir ../data/precomputed_tensors \\
        --parquet ../data/raw/daily_data_sf_v2.parquet \\
        --cache-dir ./rolling_yearly_cache_attention \\
        --start-year 2010 --end-year 2023 --device cuda

    # CPU smoke test (single year, 2 epochs):
    python tcnn_attention.py \\
        --tensor-dir ../data/precomputed_tensors \\
        --parquet ../data/raw/daily_data_sf_v2.parquet \\
        --cache-dir ./smoke_test \\
        --smoke
"""
...
def seed_everything(seed: int = 0, deterministic: bool = True):
    ...
    if deterministic:
        ...
        # warn_only=True: tolerates ops with no deterministic implementation
...
class TCNEncoder(nn.Module):
    """Dilated 1D conv encoder with single-head additive attention pooling.

    Paper Appendix A:
      - 5 conv layers, kernel=5, dilations (1,2,4,8,16), 48 hidden ch
      - ReLU + GroupNorm(4) + Dropout
      - Pooled rep dim d=96
      - Attention scorer = 2-layer MLP (hidden H/2, tanh), softmax over time
    """
    ...
    def __init__(self, in_ch: int = 3, hidden: int = 48, d: int = 96,
        ...
        super().__init__()
        if k % 2 == 0:
            raise ValueError("Use odd kernel size so causal-friendly padding keeps length stable.")
        ...
        # Single-head additive attention scorer
...
def compute_portfolio_returns_daily(w_initial: torch.Tensor, daily_rets: torch.Tensor) -> torch.Tensor:
    """Compute daily portfolio returns from initial weights with weight drift."""
...
class PrecomputedDataset:
    ...
    def get_month_data(self, idx):
        ...
        Y_t = torch.tensor(Y_raw, dtype=torch.float32).T  # (H, N)
...
def load_precomputed_data(tensor_dir: str, num_channels: int):
    ...
    if X_panel.shape[2] < num_channels:
        raise ValueError(f"Tensor has {X_panel.shape[2]} channels but --in-ch={num_channels} requested. "
                          f"Re-run prep_tensor.py with --num-channels {num_channels}.")
    if X_panel.shape[2] > num_channels:
        # Slice to first num_channels (paper config = 3)
        print(f"  slicing tensor from {X_panel.shape[2]} channels down to first {num_channels} (paper config)")

    print(f"  X_panel: {X_panel.shape}, Y_panel: {Y_panel.shape}, months: {len(holding_days)}, stocks: {len(metadata['permno_list'])}")
...
def train_one_epoch(encoder, head, dataset, opt, batch_size, device, in_ch,
    ...
    for batch_idx in range(n_full):
        ...
        for idx in range(batch_idx * batch_size, (batch_idx + 1) * batch_size):
            ...
            # Slice channels to in_ch (paper config = 3)
...
def train_model_for_year(X_panel, Y_panel, holding_days, valid_mask, metadata, date_to_idx,
    ...
    print(f"  universe: {len(top_n_permnos)} permnos")
    ...
    if train_si is None or val_si is None:
        print("  ERROR: no valid indices for train/val periods")
    ...
    encoder = TCNEncoder(in_ch=in_ch,
                          hidden=model_config["hidden"], d=model_config["d"],
    ...
    for epoch in range(max_epochs):
        ...
        if ep_returns:
            train_sharpes.append(float(np.mean(ep_returns) / (np.std(ep_returns) + 1e-8) * np.sqrt(252)))
        else:
        ...
        if val_results:
            ...
            val_sharpes.append(float(val_returns.mean() / (val_returns.std() + 1e-8) * np.sqrt(252)))
        else:
            val_sharpes.append(-999.0)
...
def run_rolling_experiment(tensor_dir, parquet_path, model_config, start_year, end_year,
                            ...
                            warmup_epochs, in_ch):
    print(f"\n{'=' * 80}\nROLLING OOS EXPERIMENT: {start_year}-{end_year}, top {top_n_stocks}\n{'=' * 80}")
    ...
    for test_year in range(start_year, end_year + 1):
        ...
        if not top_n_permnos:
            print(f"  skipping {test_year}: no universe")
            continue
    ...
    if all_yearly_results:
        ...
        sharpe = rets.mean() / (rets.std() + 1e-8) * np.sqrt(252)
        max_dd = compute_max_drawdown(rets)
        print(f"  total return: {total_ret * 100:+.2f}%   Sharpe: {sharpe:.3f}   MaxDD: {max_dd * 100:+.2f}%")
...
def parse_args():
    p = argparse.ArgumentParser(description="TCNN single-head attention training (paper variant).")
    # Paths
    p.add_argument("--tensor-dir", required=True, help="Precomputed tensor directory (output of prep_tensor.py)")
    p.add_argument("--parquet", required=True, help="daily_data_sf_v2.parquet path (for universe filtering)")
    p.add_argument("--cache-dir", default="./rolling_yearly_cache_attention")
    # Architecture (paper-faithful defaults)
    p.add_argument("--in-ch", type=int, default=3, help="Input channels (3 = paper config)")
    p.add_argument("--hidden", type=int, default=48)
    p.add_argument("--d", type=int, default=96, help="Pooled representation dim")
    p.add_argument("--kernel-size", type=int, default=5)
    p.add_argument("--dilations", type=int, nargs="+", default=[1, 2, 4, 8, 16])
    p.add_argument("--dropout", type=float, default=0.15)
    # Optimization (paper-faithful defaults)
    ...
    p.add_argument("--smoke", action="store_true",
                   help="Fast CPU sanity test: 1 year, 2 epochs, smaller min_stocks")
    return p.parse_args()
...
def main():
    args = parse_args()
    if args.smoke:
        # Override for fast sanity check
    ...
    print(f"Rolling: {args.start_year}-{args.end_year}, train={args.train_years}y, val={args.val_years}y, top-{args.top_n_stocks}")
    print(f"Training: max_epochs={args.max_epochs}, patience={args.patience}, warmup={args.warmup_epochs}, batch={args.batch_size}")
